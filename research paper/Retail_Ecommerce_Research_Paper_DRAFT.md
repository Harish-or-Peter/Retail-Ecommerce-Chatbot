...
When a query comes in, it is tokenized and passed to the fine-tuned model, which writes a reply using beam search over four beams, an n-gram repeat block (`no_repeat_ngram_size = 3`) to keep it from looping, and a ceiling of 256 new tokens. Those settings buy coherent, non-repetitive, reasonably complete answers instead of the clipped fragments greedy decoding tends to give.
